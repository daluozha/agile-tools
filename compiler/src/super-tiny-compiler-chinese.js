 ...
 *    或者其它任何东西。
 *
 * 2. *语法分析* 接收之前生成的 Token，把它们转换成一种抽象的表示，这种抽象的表示描述了代码语句中的每一个片段以及它们之间的关系。这被称为中间表示（intermediate representation）或抽象语法树（Abstract Syntax Tree， 缩写为AST）
 *
...
function tokenizer(input) {
    ...
    while (current < input.length) {
        ...
        // 下一个 token 的类型是数字。它和之前的 token 不同，因为数字可以由多个数字字符组成，但是我们只能把它们识别为一个 token。
        //
        //   (add 123 456)
        //        ^^^ ^^^
